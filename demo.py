...
say()
say("  Clone it. You get the same numbers.", 1.6)

# ---------------------------------------------------------------- 5
head("5. THE INSTRUMENT WAS WRONG TOO")
say(f"This phone is at {_live_T:.1f} C right now.", 1.0)
say(f"The defense profile caps at {_cfg0.temp_threshold:.1f} C.", 1.2)
say()
say("Mid-verification the governor fired and halted every cycle.", 1.0)
say("Three probes flipped to MISMATCH. The fix looked broken.", 1.4)
say()
say("It wasn't. Those probes were polling the live thermometer,", 1.0)
say("so a warm phone read exactly like refuted code.", 1.4)
say()
say("  A probe that reads ambient conditions is a weather report.", 1.2)
say("  Governance acts now run at an injected temperature.", 1.0)
say("  BLOCKED is a third verdict: could-not-execute is not was-wrong.", 1.6)
say()
say("And the part I did not want to find:", 1.0)
say(f"  cpu-0-1-1 idles at 44.8 C, passes 65 C under load.", 0.8)
say(f"  My own defense profile caps at {_cfg0.temp_threshold:.1f} C.", 0.8)
say("  This hardware cannot run my own profile.", 1.4)
say("  That is registered as P0. It fails on purpose, every run.", 1.8)

# ---------------------------------------------------------------- 6
head("THE LEDGER")
say("10 registered predictions. Run it yourself:", 1.2)
say()
import subprocess
out = subprocess.run([sys.executable, "probe_slc_claims.py"],
                     capture_output=True, text=True).stdout
misses = []
for line in out.splitlines():
    t = line.strip()
    if t.startswith(("P0", "P1", "P2", "P3", "P4", "P5", "P6", "P7", "P8")) \
       and ("MATCH" in t or "BLOCKED" in t):
        say("  " + t, 0.55)
        if "MISMATCH" in t:
            misses.append(t.split()[0])
    elif "predictions matched" in t:
        say()
        say("  " + t, 1.4)
say()
if misses:
    say(f"  Failing: {', '.join(misses)}.", 0.8)
    if "P0" in misses:
        say("  P0 is this phone. P1b is a legacy path I chose not to migrate.", 1.4)
    else:
        say("  P1b is a legacy path I chose not to migrate. P0 passes because", 0.8)
        say("  this substrate is currently cool enough to run the profile.", 1.4)
else:
    say("  All registered predictions matched on this run.", 1.4)
say()
